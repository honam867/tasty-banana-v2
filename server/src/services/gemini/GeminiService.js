...
class GeminiService {
  ...
  async executeWithTokens(userId, operationType, operationFn, options = {}) {
    ...
    if (userBalance.balance < tokenCost) {
      throw new Error(`Insufficient tokens. Need ${tokenCost}, have ${userBalance.balance}`);
    }
    ...
    try {
      ...
      // Update generation record with error but don't charge tokens
      await this.updateGenerationRecord(generationId, {
  ...
  async textToImage(userId, prompt, options = {}) {
    return this.executeWithTokens(userId, 'text_to_image', async () => {
      const enhancedPrompt = this.enhancePromptForProduct(prompt, options.aspectRatio);
   ...
   */
  async composeMultipleImages(userId, imagePaths, compositionPrompt, options = {}) {
  ...
  async styleTransfer(userId, contentImagePath, styleImagePath, options = {}) {
    return this.executeWithTokens(userId, 'style_transfer', async () => {
      ...
      const transferPrompt = options.customPrompt || 
  ...
  async generateWithText(userId, textContent, designPrompt, options = {}) {
    return this.executeWithTokens(userId, 'text_rendering', async () => {
      const fullPrompt = `
  ...
  enhancePromptForProduct(userPrompt, aspectRatio = null) {
    const ratioText = aspectRatio ? `Use ${aspectRatio} aspect ratio.` : '';
  ...
  getActionTemplates() {
    return {
      remove_background: 'Remove the background from this product image. Make it pure white or transparent. Keep the product sharp with clean edges and professional appearance.',
      flip_horizontal: 'Flip this image horizontally. Mirror it perfectly while maintaining the same quality and appearance.',
      flip_vertical: 'Flip this image vertically. Mirror it perfectly while maintaining the same quality and appearance.',
      enhance_lighting: 'Enhance the lighting in this product image. Make it brighter and more evenly lit while maintaining a natural, professional appearance. Soft studio lighting.',
      add_shadows: 'Add subtle professional shadows underneath the product to create depth and dimension. Keep shadows soft and natural, not harsh.',
      center_product: 'Center this product in the frame. Position it properly with good composition while keeping everything else the same.',
      pure_white_background: 'Make the background completely pure white (RGB 255,255,255). Keep the product exactly as it is, just change the background.',
        sharpen_details: 'Sharpen the details and make the image more crisp and clear. Enhance the overall sharpness while maintaining a natural look.',
        brighten_colors: 'Make the colors in this image more vibrant and bright. Enhance color saturation while keeping it professional and realistic.'
    };
  }
  ...
  checkRateLimit(userId) {
    ...
    const userRequests = this.requestHistory.filter(
      req => req.userId === userId && req.timestamp > now - this.rateLimitWindow
    );
    ...
    this.requestHistory = this.requestHistory.filter(req => req.timestamp > now - this.rateLimitWindow);
  ...
  async createGenerationRecord(data) {
    try {
      await db.insert(tokenTransactions).values({
        id: data.id,
        account_id: data.userId, // Will be linked in future schema
        type: 'debit',
        amount: data.tokensUsed,
        reason_code: 'spend_generation',
        actor_type: 'user',
      ...
      // Continue anyway - the operation will still work
    }
  }

  async updateGenerationRecord(generationId, updates) {
    try {
      // In future implementation with proper generations table:
      // await db.update(generations).set(updates).where(eq(generations.id, generationId));
      console.log(`Generation ${generationId} updated:`, updates);
    } catch (error) {
