...
export async function processTextToImage(job) {
  ...
  logger.info(`Processing text-to-image job ${job.id} for generation ${generationId}`);
  ...
  try {
    ...
    emitGenerationProgress(userId, generationId, 20, "Prompt prepared, generating images...");

    // Step 3: Generate all images sequentially
    logger.info(`Generating ${numberOfImages} images for generation ${generationId}`);
    ...
    emitGenerationProgress(userId, generationId, 80, "Uploading images to storage...");
    ...
    const imagesToUpload = generationResults.map((genResult) => ({
      ...
      title: `Generated: ${sanitizedPrompt.substring(0, 50)}... (${genResult.imageNumber}/${numberOfImages})`,
      metadata: {
        generationId,
        aspectRatio,
    ...
    emitGenerationProgress(userId, generationId, 90, "Finalizing generation...");
