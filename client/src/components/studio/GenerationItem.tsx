...
import { Loader2, AlertCircle, Clock, Coins, Image as ImageIcon } from 'lucide-react';
...
function GenerationItem({
  ...
  const { status, progress, metadata, images, tokensUsed, error, processingTimeMs } = generation;
  ...
  return (
    <>
      <motion.div
        ...
        className={`
          border-b border-white/5 p-4 transition-all min-h-[340px]
          ${isActive ? 'bg-[var(--banana-gold)]/5 border-l-4 border-l-[var(--banana-gold)]' : ''}
      ...
      >
        ...
        <div className="mb-3">
          ...
          <div className="flex items-center gap-2 text-xs text-white/40">
            ...
            <span>•</span>
            <span>{metadata.numberOfImages} image{metadata.numberOfImages > 1 ? 's' : ''}</span>
          </div>
        </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              {images.map((img, index) => (
                <button
                  ...
                  className="relative rounded-lg overflow-hidden border border-white/10 hover:border-(--banana-gold)/50 transition-colors group"
                >
                  <img
                    src={img.imageUrl}
        ...
        {isActive_status && (
          <div className="mt-3 max-h-[400px] overflow-hidden">
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-2">
              {Array.from({ length: metadata.numberOfImages }).map((_, index) => (
                <div
                  key={index}
